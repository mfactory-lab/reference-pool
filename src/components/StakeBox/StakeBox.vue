          ...
          <div class="row items-between">
            ...
            <div class="column justify-between col-sm-6 col-xs-12 q-pl-sm">
              <q-btn
                ...
                text-color="primary-gray"
                :disable="
                  connectionLost || Number(stake.from) > Number(availableSol)
                "
                @click="stakeHandler"
              >
                STAKE NOW
            ...
            <div class="col">
              <div class="stake-box__title">
                Available {{ availableXsol
                  ...
                  v-if="
                    connected && Number(unstake.from) > Number(availableXsol)
                  "
                  class="stake-box__warning gt-xs"
                  >Insufficient funds to run the transaction</span
                >
          ...
          <div class="row items-between">
            ...
            <div class="column col-sm-6 col-xs-12 q-pl-sm">
              <q-btn
                ...
                text-color="primary-gray"
                :disable="
                  connectionLost || Number(unstake.from) > Number(availableXsol)
                "
...
import { formatPct, lamportsToSol, formatAmount } from '@/utils'
...
export default defineComponent({
  ...
  setup() {
    ...
    const { cluster } = storeToRefs(useConnection())
    const { connected } = storeToRefs(useWallet())
    const { solBalance, tokenBalance } = storeToRefs(useBalance())
    const { fees, exchangeRate, connectionLost } = storeToRefs(useStakePool())
    const { depositFee, depositing, deposit } = useDeposit()
    const { withdrawFee, withdrawing, setAmount, withdraw, useWithdrawSol } =
      useWithdraw()
    const stakeAccounts = useStakeAccounts()
    const { apy } = storeToRefs(useApy())
    ...
    const withdrawAmount = computed(() => {
      ...
      let value =
        (sol - lamportsToSol(withdrawFee.value)) * (1 / exchangeRate.value)
      value -=
        value *
        (useWithdrawSol.value
    ...
    watch(withdrawAmount, () => {
      ...
      unstake.to =
        withdrawAmount.value > 0 ? withdrawAmount.value.toFixed(5) : 0
    })
    ...
    return {
      ...
      availableSol: computed(() =>
        solBalance.value ? formatAmount(solBalance.value) : '0'
      ),
